...
class AuthController {
  ...
  login = asyncHandler(async (req, res, next) => {
    ...
    // 3) generate token
    const token = createToken({ userId: user._id, role: user.role });
    user = await this.userService.updateUser(user.id, { active: true });
  ...
  protect = asyncHandler(async (req, res, next) => {
    ...
    const currentUser = await this.userService.getUserById(decoded.userId);
    if (!currentUser) {
      return next(
        new ApiError(
  ...
  // @access  Public
  verifyPassResetCode = asyncHandler(async (req, res, next) => {
